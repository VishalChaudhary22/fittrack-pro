...
/* =========================
   STORAGE
========================= */
const STORAGE_KEY = "fittrack_pro_v3";
...
const ACTIVITY = {
  ...
  extra: { label: "Extremely Active", mult: 1.9 },
};
...
const goalCals = (tdee, goal) =>
  goal === "loss" ? tdee - 500 : goal === "gain" ? tdee + 500 : tdee;
const goalProt = (w, goal) =>
  goal === "loss"
    ? Math.round(w * 2.2)
    : goal === "gain"
    ? Math.round(w * 2.0)
...
const wkLabel = (d) => {
  const dt = new Date(d);
  return `W${Math.ceil(dt.getDate() / 7)} ${dt.toLocaleDateString("en-IN", {
    month: "short",
  })}`;
};
...
const GlobalStyles = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{
      --bg:#111111;--card:#1A1A1A;--border:#252525;
      --orange:#E8540D;--orange2:rgba(232,84,13,0.12);
    ...
    input,select,textarea{
      ...
      font-family:'DM Sans',sans-serif;border-radius:8px;padding:11px 14px;
    ...
    .btn-primary{
      ...
      font-family:'DM Sans',sans-serif;font-weight:600;font-size:14px;cursor:pointer;transition:all .2s
    }
    .btn-primary:hover{background:#FF6B2B;transform:translateY(-1px)}
    .btn-ghost{
      background:transparent;border:1px solid var(--border);color:var(--t2);border-radius:8px;
      padding:8px 16px;font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer;transition:all .2s
    }
    .btn-ghost:hover{border-color:var(--orange);color:var(--orange)}
    .btn-danger{
      background:transparent;border:1px solid rgba(232,64,64,.25);color:var(--red);
      border-radius:6px;padding:5px 10px;font-size:12px;cursor:pointer;transition:all .2s
    }
    ...
    .nav-item{
      display:flex;align-items:center;gap:11px;padding:10px 14px;border-radius:10px;cursor:pointer;
      transition:all .15s;color:var(--t2);font-size:14px;font-weight:500;white-space:nowrap
    }
...
const INITIAL_SPLITS = [
  {
    ...
    name: "Push Pull Legs",
    emoji: "🔥",
    description: "6 days/week — Push Pull Legs × 2",
    color: "#E8540D",
  ...
  },
  {
    ...
    emoji: "⚡",
    description: "4 days — U L Rest U L Rest Rest",
    color: "#5B8DEF",
  ...
  },
  {
    ...
    emoji: "💥",
    description: "Full Body – Rest – Repeat",
    color: "#A855F7",
  ...
  },
  {
    ...
    emoji: "💪",
    description: "U L Rest U L Arms Rest",
    color: "#A855F7",
  ...
  },
  {
    ...
    emoji: "⚔️",
    description: "6 days FB — lower volume per session",
    color: "#4CAF50",
  ...
  },
  {
    ...
    name: "Home Workouts",
    emoji: "🏠",
    description: "Bodyweight training + Yoga",
    color: "#4CAF50",
    ...
    days: [
      {
        ...
        exercises: [
          ...
          { id: genId(), name: "Pike Push-ups", sets: 3, repsRange: "10-15", muscle: "Shoulders", notes: "Hips high" },
      ...
      },
      {
        ...
        type: "yoga",
        exercises: [
          { id: genId(), name: "Surya Namaskar", sets: 5, repsRange: "1 round", muscle: "Full Body", notes: "12 poses per round" },
          { id: genId(), name: "Downward Dog", sets: 3, repsRange: "5 breaths", muscle: "Hamstrings/Back", notes: "" },
          { id: genId(), name: "Warrior I", sets: 3, repsRange: "5 breaths each side", muscle: "Hips/Legs", notes: "" },
          { id: genId(), name: "Bridge Pose", sets: 3, repsRange: "8-10 breaths", muscle: "Glutes/Back", notes: "" },
          { id: genId(), name: "Child's Pose", sets: 1, repsRange: "10 breaths", muscle: "Recovery", notes: "" },
        ],
      },
      {
        id: "home-int",
...
const DIET_TYPES = {
  vegan: {
    ...
    icon: "🌱",
    color: "#4CAF50",
    proteinSources: ["Tofu", "Tempeh", "Lentils", "Chickpeas", "Soy milk", "Moong dal", "Rajma", "Pea protein", "Seeds"],
    meals: [
      { time: "7:00 AM", label: "Breakfast", icon: "🌅", items: ["Overnight oats with soy milk, banana & chia", "Besan chilla with mint chutney"] },
      { time: "1:00 PM", label: "Lunch", icon: "🍱", items: ["2 rotis", "Rajma/chana masala", "Seasonal veg", "Salad"] },
      { time: "4:30 PM", label: "Pre-Workout", icon: "⚡", items: ["Banana + peanut butter", "Sattu drink"] },
      { time: "7:30 PM", label: "Dinner", icon: "🍽️", items: ["Brown rice or 2 rotis", "Tofu bhurji or moong dal", "Mixed veg"] },
    ],
  },
  vegetarian: {
    id: "vegetarian",
    ...
    icon: "🥛",
    color: "#5B8DEF",
    proteinSources: ["Paneer", "Curd", "Milk", "Lentils", "Rajma", "Chana", "Soy chunks"],
    meals: [
      { time: "7:00 AM", label: "Breakfast", icon: "🌅", items: ["Moong dal chilla with curd", "Poha with peanuts"] },
      { time: "1:00 PM", label: "Lunch", icon: "🍱", items: ["Rotis with ghee", "Paneer bhurji or palak paneer", "Dal + salad"] },
      { time: "4:30 PM", label: "Pre-Workout", icon: "⚡", items: ["Greek yogurt with banana", "Black coffee"] },
      { time: "7:30 PM", label: "Dinner", icon: "🍽️", items: ["Brown rice / roti", "Soya chunks curry", "Greens + raita"] },
    ],
  },
  egg: {
    id: "egg",
    ...
    icon: "🥚",
    color: "#E8540D",
    proteinSources: ["Eggs", "Paneer", "Curd", "Milk", "Lentils", "Chickpeas"],
    meals: [
      { time: "7:00 AM", label: "Breakfast", icon: "🌅", items: ["3 egg whites + 1 whole egg", "2 toasts or 1 roti", "Milk"] },
      { time: "1:00 PM", label: "Lunch", icon: "🍱", items: ["2 rotis or brown rice", "Egg curry or dal", "Veg sabzi + salad"] },
      { time: "4:30 PM", label: "Pre-Workout", icon: "⚡", items: ["2 boiled eggs + banana", "Sattu or nimbu pani"] },
      { time: "7:30 PM", label: "Dinner", icon: "🍽️", items: ["Egg bhurji or omelette", "Roti + sabzi", "Buttermilk"] },
    ],
  },
  nonveg: {
    id: "nonveg",
    ...
    icon: "🍗",
    color: "#E8540D",
    proteinSources: ["Chicken", "Fish", "Eggs", "Paneer", "Curd"],
    meals: [
      { time: "7:00 AM", label: "Breakfast", icon: "🌅", items: ["Egg scramble", "Toast/roti", "Milk + almonds"] },
      { time: "1:00 PM", label: "Lunch", icon: "🍱", items: ["Brown rice or rotis", "Chicken curry / fish curry", "Dal + salad"] },
      { time: "4:30 PM", label: "Pre-Workout", icon: "⚡", items: ["Banana + peanut butter", "Black coffee"] },
      { time: "7:30 PM", label: "Dinner", icon: "🍽️", items: ["Grilled chicken / fish", "Roti or rice", "Vegetables + buttermilk"] },
    ],
  },
};
...
/* =========================
   INITIAL APP DATA
========================= */
const makeSampleHealthLogs = () => {
  const logs = [];
  const today = new Date();
  for (let i = 55; i >= 0; i--) {
    const d = new Date(today);
    d.setDate(d.getDate() - i);
    const w = +(88 - (55 - i) * 0.09 + (Math.random() - 0.5) * 0.8).toFixed(1);
    if (i % 2 === 0 || i < 7) {
      logs.push({
        id: genId(),
        userId: "vishal",
      ...
      exercises: [
        {
          ...
          sets: [
            { reps: 12, weight: bwArr[w] },
            { reps: 11, weight: bwArr[w] },
            { reps: 10, weight: bwArr[w] },
          ],
        },
        {
          name: "Overhead Press",
          sets: [
            { reps: 10, weight: 40 + w * 2.5 },
            { reps: 9, weight: 40 + w * 2.5 },
          ],
        },
      ],
    });
...
const MetricCard = ({ label, value, unit, icon, color = "#E8540D", sub, trend, small }) => (
  <div className="card fade-up" style={{ padding: small ? "14px 18px" : "20px 22px", position: "relative", overflow: "hidden" }}>
    ...
    <div style={{ fontSize: 11, color: "var(--t3)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 4 }}>
      {label}
    </div>
    <div style={{ fontFamily: "Bebas Neue, sans-serif", fontSize: small ? 28 : 38, color, lineHeight: 1, letterSpacing: "1px" }}>
      {value}
      <span style={{ fontSize: small ? 13 : 16, fontWeight: 400, color: "var(--t2)", marginLeft: 3 }}>{unit}</span>
    </div>
    {sub && <div style={{ fontSize: 11, color: "var(--t2)", marginTop: 4 }}>{sub}</div>}
    {trend !== undefined && (
      <div style={{ fontSize: 11, marginTop: 4, color: trend > 0 ? "var(--red)" : "var(--green)" }}>
...
const AuthModal = ({ users, setUsers, onLogin }) => {
  ...
  const handleRegister = () => {
    ...
    const u = {
      ...
      workoutDays: parseInt(form.workoutDays) || 4,
      isAdmin: false,
      activeSplitId: "ppl",
      joinDate: todayStr(),
      bio: "",
    ...
    <div className="modal-overlay" style={{ background: "radial-gradient(ellipse at 60% 80%, rgba(232,84,13,.15) 0%, rgba(0,0,0,.98) 70%)" }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: "var(--orange)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", fontSize: 26 }}>
            💪
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
            {mode === "register" && (
              <div>
                ...
                <input placeholder="Vishal Chaudhary" value={form.name} onChange={set("name")} />
              </div>
            )}
            <div>
              <label>Email *</label>
            ...
            {err && (
              <div style={{ color: "var(--red)", fontSize: 12, background: "rgba(232,64,64,.08)", padding: "9px 12px", borderRadius: 8, border: "1px solid rgba(232,64,64,.15)" }}>
                {err}
              </div>
            )}
            ...
            <button
              ...
              style={{ width: "100%", padding: "14px", fontSize: 15, borderRadius: 10, marginTop: 2 }}
...
const DashboardPage = ({ user, healthLogs, workoutLogs, splits, setHealthLogs }) => {
  ...
  const wkData = useMemo(() => {
    const sorted = [...healthLogs]
      .filter((l) => l.userId === user.id)
      .sort((a, b) => new Date(a.date) - new Date(b.date));
    const byWk = {};
    sorted.forEach((l) => {
      const wk = wkLabel(l.date);
      if (!byWk[wk]) byWk[wk] = [];
      byWk[wk].push(l.weight);
    });
    return Object.entries(byWk).map(([week, ws]) => ({
      week,
      weight: +(ws.reduce((a, b) => a + b, 0) / ws.length).toFixed(1),
    }));
  }, [healthLogs, user.id]);
  ...
  const trend =
    wkData.length >= 2
      ? +(wkData[wkData.length - 1].weight - wkData[wkData.length - 2].weight).toFixed(1)
      : undefined;
    ...
    <div className="fade-up">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 16, marginBottom: 16 }} className="grid-2">
        <div className="card" style={{ padding: 20 }}>
          <div style={{ fontSize: 13, color: "var(--t3)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 14 }}>
            Weight Trend
          </div>
            ...
            <AreaChart data={wkData}>
              <defs>
                <linearGradient id="og" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#E8540D" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#E8540D" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="weight" stroke="#E8540D" strokeWidth={2} fill="url(#og)" dot={{ fill: "#E8540D", r: 3 }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>

        <div className="card" style={{ padding: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <div style={{ fontSize: 13, color: "var(--t3)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 16 }}>
            BMI Status
          </div>
          <div className="section-title" style={{ fontSize: 64, color: bmiCat.color, lineHeight: 1 }}>
            {bmi || "—"}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 16, width: "100%" }}>
            {[
              { l: "Under", r: "<18.5", c: "#5B8DEF" },
              { l: "Normal", r: "18.5-25", c: "#4CAF50" },
              { l: "Over", r: "25-30", c: "#E8540D" },
              { l: "Obese", r: ">30", c: "#E84040" },
            ].map((s) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16 }} className="grid-2">
        <div className="card" style={{ padding: 20 }}>
          <div style={{ fontSize: 13, color: "var(--t3)", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 12 }}>
            Active Split
          </div>
          {activeSplit ? (
            <>
                ...
                {activeSplit.schedule.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 10,
                      fontWeight: 600,
                      background: d === "Rest" ? "#111" : activeSplit.color + "15",
                      color: d === "Rest" ? "var(--t3)" : activeSplit.color,
                      border: `1px solid ${d === "Rest" ? "var(--border)" : activeSplit.color + "35"}`,
                    }}
                  >
                    D{i + 1}:{d}
                  </div>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          {recent.length === 0 ? (
            <div style={{ color: "var(--t3)", fontSize: 13 }}>No sessions logged yet.</div>
          ) : (
            recent.map((w) => (
              <div key={w.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 0", borderBottom: "1px solid var(--border)" }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "var(--t2)", marginTop: 2 }}>
                    {fmtDate(w.date)} · {w.exercises?.length || 0} exercises
                  </div>
                </div>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 380 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button className="btn-ghost" onClick={() => setShowLog(false)} style={{ padding: "5px 9px" }}>
                <X size={14} />
              </button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                <label>Weight (kg)</label>
                <input type="number" step=".1" value={nw.weight} onChange={(e) => setNw((p) => ({ ...p, weight: e.target.value }))} />
              </div>
              <div>
                <label>Notes (optional)</label>
                <input placeholder="Post-morning, post-workout..." value={nw.notes} onChange={(e) => setNw((p) => ({ ...p, notes: e.target.value }))} />
              </div>
              <button className="btn-primary" style={{ width: "100%", padding: "13px" }} onClick={logWeight}>
                Save
              </button>
            </div>
          </div>
...
const SplitsPage = ({ user, splits, setSplits, setActiveSplitId, setPage, isAdmin }) => {
  ...
  const handleSelectSplit = (splitId) => {
    ...
    setTimeout(() => setPage("workout"), 200);
  };

  const updEx = (sId, dId, eId, data) => {
    setSplits((prev) =>
      prev.map((split) => {
        if (split.id !== sId) return split;
        return {
          ...split,
          days: split.days.map((day) => {
            if (day.id !== dId) return day;
            return {
              ...day,
              exercises: day.exercises.map((ex) => (ex.id !== eId ? ex : { ...ex, ...data })),
            };
          }),
        };
      })
    ...
    setSplits((prev) =>
      prev.map((split) => {
        ...
        return {
          ...
          days: split.days.map((day) => {
            if (day.id !== dId) return day;
            return { ...day, exercises: day.exercises.filter((ex) => ex.id !== eId) };
          }),
        };
      })
    );
    ...
    setSplits((prev) =>
      prev.map((split) => {
        ...
        return {
          ...
          days: split.days.map((day) => {
            if (day.id !== dId) return day;
            return { ...day, exercises: [...day.exercises, e] };
          }),
        };
      })
    );
  ...
  const addSplit = () => {
    const split = {
      ...
      schedule: ["Day 1", "Rest"],
      days: [
        { id: genId(), name: "Day 1", type: "custom", exercises: [] },
        { id: genId(), name: "Rest", type: "rest", exercises: [] },
      ],
    };
    setSplits((prev) => [...prev, split]);
    setNs({ name: "", emoji: "💪", description: "", color: "#E8540D" });
  ...
  return (
    <div className="fade-up">
      <PageHeader
        ...
        subtitle="Select a program — you'll go straight to the tracker"
          ...
          <div style={{ padding: "18px 20px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              {split.comingSoon ? (
                ...
                <button className="btn-primary" style={{ padding: "8px 16px", fontSize: 13 }} onClick={() => handleSelectSplit(split.id)}>
                  Select →
                </button>
              )}
              <ChevronDown
                size={15}
                color="var(--t3)"
                style={{ transform: expanded === split.id ? "rotate(180deg)" : "", transition: ".2s", flexShrink: 0 }}
            ...
            <div style={{ borderTop: "1px solid var(--border)", padding: "16px 20px 20px" }}>
                ...
                {split.schedule.map((d, i) => (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 10,
                      fontWeight: 700,
                      background: d === "Rest" ? "#111" : split.color + "12",
                      color: d === "Rest" ? "var(--t3)" : split.color,
                      border: `1px solid ${d === "Rest" ? "var(--border)" : split.color + "30"}`,
                    }}
                  >
                    D{i + 1}: {d}
                  </div>
              ...
              {split.days.filter((d) => d.type !== "rest").map((day) => (
                <div key={day.id} style={{ background: "#111", borderRadius: 10, marginBottom: 8, overflow: "hidden" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 16px", cursor: "pointer" }} onClick={() => setExpandDay(expandDay === day.id ? null : day.id)}>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              ...
                              {isAdmin && (
                                <div style={{ display: "flex", gap: 5 }}>
                                  ...
                                  <button className="btn-danger" style={{ padding: "3px 7px" }} onClick={() => delEx(split.id, day.id, ex.id)}>
                                    <Trash2 size={11} />
                                  </button>
                                </div>
                              )}
                        ...
                        (showAddEx === day.id ? (
                          <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              <button className="btn-primary" style={{ padding: "8px 16px", fontSize: 13 }} onClick={() => addEx(split.id, day.id)}>
                                Add
                              </button>
                              <button className="btn-ghost" style={{ fontSize: 13 }} onClick={() => setShowAddEx(null)}>
                                Cancel
                              </button>
                            </div>
                          </div>
                        ) : (
                          <button className="btn-ghost" style={{ marginTop: 10, width: "100%", fontSize: 12 }} onClick={() => setShowAddEx(day.id)}>
                            + Add Exercise
                          </button>
                        ))}
                    </div>
          ...
          {expanded === split.id && split.comingSoon && (
            <div style={{ borderTop: "1px solid var(--border)", padding: 40, textAlign: "center" }}>
              ...
              <div style={{ color: "var(--t2)", fontSize: 13, marginTop: 6 }}>
                Powerlifting programming with Squat, Bench & Deadlift cycles is in development.
              </div>
            </div>
          )}
        </div>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: 440 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button className="btn-ghost" style={{ padding: "5px 9px" }} onClick={() => setShowAddSplit(false)}>
                <X size={14} />
              </button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
              <div>
                <label>Name</label>
                <input placeholder="e.g. Bro Split" value={ns.name} onChange={(e) => setNs((p) => ({ ...p, name: e.target.value }))} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div>
                  <label>Emoji</label>
                  <input value={ns.emoji} onChange={(e) => setNs((p) => ({ ...p, emoji: e.target.value }))} />
                </div>
                <div>
                  <label>Color</label>
                  <input type="color" value={ns.color} onChange={(e) => setNs((p) => ({ ...p, color: e.target.value }))} />
                </div>
              </div>
              <div>
                <label>Description</label>
                <input placeholder="5 days/week..." value={ns.description} onChange={(e) => setNs((p) => ({ ...p, description: e.target.value }))} />
              </div>
              <button className="btn-primary" style={{ width: "100%", padding: "13px" }} onClick={addSplit}>
                Create Split
              </button>
            </div>
          </div>
...
const WorkoutPage = ({ user, splits, workoutLogs, setWorkoutLogs }) => {
  ...
  const startSession = (day) => {
    const exercises = day.exercises.map((ex) => {
      const prev = workoutLogs
        .filter((l) => (l.userId === user.id || l.userId === "vishal") && l.dayId === day.id)
      ...
      return {
        ...
        sets: Array.from({ length: ex.sets }, (_, i) => {
          ...
          return {
            reps: ps?.reps || ex.repsRange?.split("-")[0] || 8,
            weight: ps?.weight || 0,
            done: false,
          };
        }),
  ...
  const addSet = (ei) =>
    setSession((p) => {
      ...
      exs[ei] = { ...exs[ei], sets: [...exs[ei].sets, { ...last, done: false }] };
      return { ...p, exercises: exs };
    });
  ...
  const setVariant = (ei, v) =>
    setSession((p) => {
      const exs = [...p.exercises];
      exs[ei] = { ...exs[ei], selectedVariant: v };
      return { ...p, exercises: exs };
    });
  ...
  if (done) {
    return (
      <div className="fade-up" style={{ textAlign: "center", padding: "80px 20px" }}>
        ...
        <button className="btn-primary" style={{ padding: "14px 32px", fontSize: 16 }} onClick={() => { setSession(null); setDone(false); }}>
          Log Another
        </button>
      </div>
    );
    ...
    return (
      <div className="fade-up">
          ...
          <div key={ex.id} className="card" style={{ marginBottom: 14, padding: 18 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div>
                ...
                {ex.variants && (
                  <select
                    value={ex.selectedVariant || ex.variants[0]}
                    onChange={(e) => setVariant(ei, e.target.value)}
              ...
              <button className="btn-ghost" style={{ fontSize: 12, padding: "5px 10px" }} onClick={() => addSet(ei)}>
                + Set
              </button>
            </div>
            ...
            {ex.sets.map((s, si) => (
              <div key={si} className="exercise-row" style={{ marginBottom: 6, opacity: s.done ? 0.7 : 1 }}>
                ...
                <div style={{ display: "flex", gap: 4 }}>
                  <button
                    onClick={() => updSet(ei, si, "done", !s.done)}
                    style={{
                      flex: 1,
                      background: s.done ? "var(--orange2)" : "#111",
                      border: `1px solid ${s.done ? "var(--orange)" : "var(--border)"}`,
                      borderRadius: 8,
                      color: s.done ? "var(--orange)" : "var(--t3)",
                      cursor: "pointer",
                      padding: "8px 0",
                      fontSize: 15,
                      transition: "all .15s",
        ...
        <div className="card" style={{ marginBottom: 14, padding: 16 }}>
          ...
          <textarea
            rows={2}
            placeholder="PRs, form notes, how it felt..."
            value={session.notes}
    ...
    <div className="fade-up">
      ...
      {!activeSplit ? (
        <div style={{ textAlign: "center", padding: 40 }}>
          <div style={{ color: "var(--t2)" }}>Go to Splits to select a program first</div>
        </div>
      ) : (
...
const DietPage = ({ user }) => {
  ...
  const carbs = Math.round((cals * (goal === "loss" ? 0.4 : 0.45)) / 4);
  ...
  const GOALS = [
    { id: "loss", label: "Weight Loss", icon: "🔥", color: "#E84040" },
    { id: "maintenance", label: "Maintenance", icon: "⚖️", color: "#E8540D" },
    { id: "gain", label: "Weight Gain", icon: "💪", color: "#4CAF50" },
  ];
    ...
    <div className="fade-up">
      ...
      <div className="card" style={{ padding: 18, marginBottom: 18, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {[
          ...
          { l: "Activity", v: ACTIVITY[user.activityLevel || "moderate"]?.label },
          { l: "BMR", v: `${bmr} kcal` },
          { l: "TDEE", v: `${tdee} kcal` },
        ].map((s) => (
        ...
        {GOALS.map((g) => (
          <div
            ...
            style={{
              padding: 16,
              cursor: "pointer",
              border: `1px solid ${goal === g.id ? g.color : "var(--border)"}`,
              background: goal === g.id ? `${g.color}08` : "var(--card)",
              transition: "all .2s",
            }}
          ...
          >
            ...
            <div style={{ fontSize: 11, color: g.color, marginTop: 4, fontWeight: 600 }}>
              {g.id === "loss" ? `${cals} kcal/day` : g.id === "gain" ? `${cals} kcal/day` : `${tdee} kcal/day`}
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10, marginBottom: 18 }}>
        {[
          { l: "Calories", v: cals, u: "kcal", c: "var(--orange)" },
          { l: "Protein", v: prot, u: "g", c: "#5B8DEF" },
          { l: "Carbs", v: carbs, u: "g", c: "#4CAF50" },
          { l: "Fat", v: fat, u: "g", c: "#A855F7" },
        ].map((m) => (
          <div key={m.l} className="card" style={{ padding: 14, textAlign: "center" }}>
            ...
            <div className="section-title" style={{ fontSize: 26, color: m.c }}>
              {m.v}<span style={{ fontSize: 12, color: "var(--t2)" }}>{m.u}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        {Object.values(DIET_TYPES).map((d) => (
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              transition: "all .2s",
              background: dietType === d.id ? d.color : "transparent",
              color: dietType === d.id ? "#fff" : "var(--t2)",
      ...
      <div className="card" style={{ padding: 16, marginBottom: 16, borderLeft: `3px solid ${dt.color}` }}>
        <div style={{ fontSize: 12, color: "var(--t3)", fontWeight: 700, textTransform: "uppercase", marginBottom: 8 }}>
          Best Protein Sources — {dt.label}
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {dt.proteinSources.map((s) => (
            <span key={s} style={{ padding: "4px 10px", background: `${dt.color}10`, color: dt.color, border: `1px solid ${dt.color}25`, borderRadius: 20, fontSize: 12, fontWeight: 500 }}>
              {s}
            </span>
          ))}
        </div>
            ...
            {meal.items.map((item, j) => (
              <div
                ...
                style={{
                  ...
                  fontSize: 12,
                  color: "#CCC",
                  borderBottom: j < meal.items.length - 1 ? "1px solid #1E1E1E" : "",
                }}
              >
                <span style={{ color: dt.color, flexShrink: 0, marginTop: 1 }}>•</span>
                {item}
              </div>
            ))}
          </div>
      ...
      <div className="card" style={{ marginTop: 14, padding: 14, borderLeft: "3px solid var(--orange)" }}>
        <div style={{ fontSize: 11, color: "var(--t2)" }}>
          📌 <strong>Note:</strong> Scale portions up or down to hit your daily calorie target of{" "}
          <strong style={{ color: "var(--orange)" }}>{cals} kcal</strong>.
        </div>
      </div>
    </div>
  );
...
const ProgressPage = ({ user, workoutLogs, splits }) => {
  ...
  const allExNames = useMemo(() => {
    const ns = new Set();
    userLogs
      .filter((l) => !selDay || l.dayId === selDay)
      .forEach((l) => l.exercises?.forEach((e) => ns.add(e.name)));
      ...
      {selEx && chartData.length > 0 ? (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 18 }}>
            ...
            {chartData.length >= 2 && (
              <MetricCard label="Total Progress" value={`+${(chartData[chartData.length - 1].maxWeight - chartData[0].maxWeight).toFixed(1)}`} unit="kg" icon="📈" color="#4CAF50" small />
            )}
          </div>
            ...
            <div className="card" style={{ padding: 20 }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <AreaChart data={chartData}>
                  ...
                  <YAxis tick={{ fill: "#666", fontSize: 10 }} />
                  <Tooltip contentStyle={{ background: "#1A1A1A", border: "1px solid #252525", borderRadius: 8, fontSize: 12 }} />
                  <Area type="monotone" dataKey="maxWeight" stroke="#E8540D" strokeWidth={2} fill="url(#cg1)" dot={{ fill: "#E8540D", r: 4 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="card" style={{ padding: 20 }}>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <LineChart data={chartData}>
                  ...
                  <YAxis tick={{ fill: "#666", fontSize: 10 }} />
                  <Tooltip contentStyle={{ background: "#1A1A1A", border: "1px solid #252525", borderRadius: 8, fontSize: 12 }} />
                  <Line type="monotone" dataKey="avgReps" stroke="#4CAF50" strokeWidth={2} dot={{ fill: "#4CAF50", r: 4 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
  ...
  if (submitted) {
    return (
      <div className="fade-up" style={{ textAlign: "center", padding: "80px 20px" }}>
        ...
        <div style={{ color: "var(--t2)", marginTop: 8, marginBottom: 8 }}>Your email client should have opened.</div>
        <button className="btn-primary" style={{ padding: "13px 28px" }} onClick={() => setSubmitted(false)}>
          Send Another
        </button>
      </div>
    );
    ...
    <div className="fade-up">
      ...
      <div className="card" style={{ padding: 24 }}>
        ...
        <div style={{ marginTop: 14 }}>
          <label>Tell me about yourself & your goals</label>
          <textarea rows={4} placeholder="Current weight, training experience, dietary preferences, schedule..." value={form.message} onChange={set("message")} style={{ resize: "vertical" }} />
        </div>
        <button className="btn-primary" style={{ marginTop: 18, padding: "14px 28px", fontSize: 15 }} onClick={handleSubmit} disabled={!form.name || !form.email}>
          Send Inquiry →
        </button>
      </div>
    </div>
...
/* =========================
   PROFILE PAGE
========================= */
const ProfilePage = ({ user, setUsers, onLogout, splits }) => {
      ...
      p.map((u) =>
        u.id === user.id
          ? {
              ...
              workoutDays: parseInt(form.workoutDays),
            }
          : u
      )
    );
    ...
    <div className="fade-up">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "260px 1fr", gap: 16 }} className="grid-2">
        <div className="card" style={{ padding: 24, textAlign: "center", height: "fit-content" }}>
          <div style={{ width: 72, height: 72, borderRadius: "50%", background: "var(--orange)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: "#fff", letterSpacing: "2px" }}>
            {user.avatar}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            {[
              ...
              <div key={f.k}>
                ...
                {editing ? (
                  ...
                  <div style={{ padding: "10px 14px", background: "#111", borderRadius: 8, fontSize: 14, border: "1px solid var(--border)" }}>
                    {String(user[f.k])}
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              {editing ? (
                ...
                <div style={{ padding: "10px 14px", background: "#111", borderRadius: 8, fontSize: 13, border: "1px solid var(--border)", color: "var(--t2)" }}>
                  {ACTIVITY[user.activityLevel || "moderate"]?.label}
                </div>
              )}
            </div>
          </div>

          <div className="card" style={{ marginTop: 18, padding: 16, background: "#111" }}>
            <div className="section-title" style={{ fontSize: 18, marginBottom: 8 }}>Share Your App</div>
            <div style={{ fontSize: 12, color: "var(--t2)", marginBottom: 10 }}>Share this URL on Instagram so people can use the app.</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <input value={shareLink} readOnly />
              <button className="btn-ghost" onClick={() => navigator.clipboard.writeText(shareLink)}>
                <Share2 size={14} /> Copy
              </button>
            </div>
          </div>
...
const NAV = [
  ...
  { id: "workout", label: "Tracker", Icon: () => <span style={{ fontSize: 18 }}>🏋️</span> },
...
export default function App() {
  ...
  const [splits, setSplits] = useState(saved?.splits || INITIAL_SPLITS);
  const [healthLogs, setHealthLogs] = useState(saved?.healthLogs || makeSampleHealthLogs());
  const [workoutLogs, setWorkoutLogs] = useState(saved?.workoutLogs || makeSampleWorkoutLogs());
  const [sidebarOpen, setSidebarOpen] = useState(true);

  useEffect(() => {
    saveState({ users, currentUserId, page, splits, healthLogs, workoutLogs });
  }, [users, currentUserId, page, splits, healthLogs, workoutLogs]);
  ...
  const setActiveSplit = (id) =>
    setUsers((p) => p.map((u) => (u.id === currentUserId ? { ...u, activeSplitId: id } : u)));
  ...
  const renderPage = () => {
    ...
    switch (page) {
      ...
      case "splits":
        return <SplitsPage {...props} setActiveSplitId={setActiveSplit} setPage={setPage} isAdmin={user.isAdmin} />;
      case "workout":
      ...
      case "contact":
        return <ContactPage />;
      case "profile":
        return <ProfilePage user={user} setUsers={setUsers} onLogout={handleLogout} splits={splits} />;
      default:
        return <DashboardPage {...props} />;
    }
  };
    ...
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        >
          ...
          >
            <div style={{ width: 32, height: 32, borderRadius: 8, background: "var(--orange)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>
              💪
            </div>
            {sidebarOpen && (
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 18, letterSpacing: "2px", color: "var(--orange)", whiteSpace: "nowrap" }}>
                FITTRACK PRO
              </div>
            )}
          </div>
          ...
          {sidebarOpen && (
            <div style={{ padding: "14px", borderBottom: "1px solid var(--border)", display: "flex", gap: 10, alignItems: "center" }}>
              <div style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--orange)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Bebas Neue',sans-serif", fontWeight: 900, color: "#fff", fontSize: 13, flexShrink: 0 }}>
                {user.avatar}
              </div>
          ...
          {sidebarOpen && (
            <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border)", fontSize: 10, color: "var(--t3)" }}>
              FitTrack Pro · Share on Instagram 📸
            </div>
          )}
        </div>
        ...
        <nav className="bottom-nav">
          {[NAV[0], NAV[1], NAV[2], NAV[3], NAV[6]].map(({ id, label, Icon }) => (
            <button
              key={id}
              onClick={() => setPage(id)}
              style={{
                ...
                cursor: "pointer",
                padding: "6px 0",
                color: page === id ? "var(--orange)" : "var(--t3)",
                transition: "color .15s",
              }}
            >
              <Icon size={20} />
              <span style={{ fontSize: 9, fontWeight: 600, letterSpacing: ".3px", textTransform: "uppercase" }}>
                {label}
              </span>
            </button>
          ))}
